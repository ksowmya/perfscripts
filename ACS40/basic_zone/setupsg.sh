...
#create the zone
zone_query="GET  http://$host/client/?command=createZone&networktype=Basic&securitygroupenabled=true&name=BLRBasic&dns1=4.2.2.2&internaldns1=4.2.2.2  HTTP/1.0\n\n"
createzone_out=$(echo -e $zone_query | nc -v -w $delay $host 8096)
echo $createzone_out
zoneid=$(echo $createzone_out | sed 's/\(.*<id>\)\(.*\)\(<\/id>.*\)/\2/g')

physicalNetwork_query="GET http://$host/client/?command=createPhysicalNetwork&zoneid=$zoneid&name=Physical_Network_1 HTTP/1.0\n\n"
physicalNetwork_out=$(echo -e $physicalNetwork_query | nc -v -w $delay $host 8096)
echo $physicalNetwork_out
jobid=$(echo $physicalNetwork_out | sed 's/\(.*<jobid>\)\(.*\)\(<\/jobid>.*\)/\2/g')
getidquery="GET http://$host/client/?command=queryAsyncJobResult&jobid=$jobid HTTP/1.1\n\n"
...
addmgmttraffictype_query="GET http://$host/client/?command=addTrafficType&trafficType=Management&physicalnetworkid=$phynetworkid HTTP/1.0\n\n"
echo -e $addmgmttraffictype_query | nc -v -w $delay $host 8096

addguesttraffictype_query="GET http://$host/client/?command=addTrafficType&trafficType=Guest&physicalnetworkid=$phynetworkid HTTP/1.0\n\n"
echo -e $addguesttraffictype_query | nc -v -w $delay $host 8096

update_physicalNetwork_query="GET http://$host/client/?command=updatePhysicalNetwork&state=Enabled&id=$phynetworkid HTTP/1.0\n\n"
echo -e $update_physicalNetwork_query | nc -v -w $delay $host 8096

listNetworkServiceProviders_query="GET http://$host/client/?command=listNetworkServiceProviders&name=VirtualRouter&physicalNetworkId=$phynetworkid HTTP/1.0\n\n"
...
listNetworkServiceProviders_query="GET http://$host/client/?command=listNetworkServiceProviders&name=SecurityGroupProvider&physicalNetworkId=$phynetworkid HTTP/1.0\n\n"
echo $listNetworkServiceProviders_query
listNetworkServiceProviders_out=$(echo -e $listNetworkServiceProviders_query  | nc -v -w $delay $host 8096)
echo $listNetworkServiceProviders_out
nsp_id=$(echo $listNetworkServiceProviders_out | sed 's/\(.*<id>\)\(.*\)\(<\/id>.*\)/\2/g')
...
createNetwork_query="GET http://$host/client/?command=createNetwork&zoneid=$zoneid&name=guestNetworkForBasicZone&displaytext=guestNetworkForBasicZone&networkofferingid=5 HTTP/1.0\n\n"
createNetwork_out=$(echo -e $createNetwork_query | nc -v -w $delay $host 8096)
network_id=$(echo $createNetwork_out| sed 's/\(.*<id>\)\(.*\)\(<\/id>.*\)/\2/g')

updateZone_query="GET http://$host/client/?command=updateZone&allocationstate=Enabled&id=$zoneid HTTP/1.0\n\n"
